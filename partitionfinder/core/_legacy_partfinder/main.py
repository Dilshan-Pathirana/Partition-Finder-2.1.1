...
def check_python_version():
    """Check the python version is 3.8 or higher"""

    version_major = sys.version_info[0]
    version_minor = sys.version_info[1]
    
    log.info("You have Python version %d.%d" % (version_major, version_minor))

    if version_major < 3 or (version_major == 3 and version_minor < 8):
        log.error("""
            Your Python version is %d.%d, but this program requires Python 3.8 or higher.
            Please upgrade to version 3.8+ by visiting www.python.org/downloads,
            or by following the instructions in the PartitionFinder manual.
            """ % (version_major, version_minor))
        return 0
    else:
        log.info("Python 3.8+ detected - good!")
        return 1
