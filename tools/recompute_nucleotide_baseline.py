...
def _copy_example_to_tmp(input_dir: Path, tmp_path: Path) -> Path:
    # Use a unique temp folder each run to avoid Windows file-lock issues when
    # previous runs still have files open (e.g. antivirus/indexers).
    stamp = f"{int(time.time())}_{os.getpid()}"
    dst = tmp_path / f"{input_dir.name}_{stamp}"
...
def main() -> int:
    ...
    spec = baseline["nucleotide"]

    input_dir = REPO_ROOT / spec["input_dir"]
    work_dir = _copy_example_to_tmp(input_dir, REPO_ROOT / ".tmp_baseline")
